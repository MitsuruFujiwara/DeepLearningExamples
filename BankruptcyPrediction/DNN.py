...
def _model():
    model = Sequential()
    model.add(Dense(output_dim=64, input_dim=64, W_regularizer=l2(0.0001)))
    ...
    model.add(Activation('relu'))
    model.add(Dropout(0.5))
    model.add(Dense(output_dim=1, input_dim=64, W_regularizer=l2(0.0001)))
...
def main():
    ...
    df_val = pd.read_hdf('data.h5', key='data5_test', mode='r')
