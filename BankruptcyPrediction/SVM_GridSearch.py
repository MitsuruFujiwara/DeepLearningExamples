...
def main():
    ...
    prm_gamma = np.power(10.0, np.arange(-5,6,1))
    prm_C = np.power(10.0, np.arange(-4,5,1))

    # set prarameter grid
    param_grid = [{'C':prm_C,'gamma':prm_gamma}]

    # grid search
    gs = GridSearchCV(estimator=svc, param_grid=param_grid, scoring='roc_auc', cv=3, n_jobs=-1)
    ...
    result.to_csv('result_svm5.csv')

    joblib.dump(gs.best_estimator_, 'svm5.pkl')
    ...
    plt.title('Year5 AUC Score')
    plt.savefig('GridSearch5.eps')
    plt.show()
