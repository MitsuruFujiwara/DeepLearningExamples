...
def main():
    ...
    solver_range = ['newton-cg', 'lbfgs', 'sag'] # set solver range
    ...
    gs = GridSearchCV(estimator=lr, param_grid=param_grid, scoring='roc_auc', cv=3, n_jobs=-1)

    gs.fit(trX, trY)

    result = pd.DataFrame(gs.cv_results_)

    result.to_csv('result_lr5.csv')

    joblib.dump(gs.best_estimator_, 'lr5.pkl')
