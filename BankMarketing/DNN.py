...
def _model():
    ...
    model.add(Activation('relu'))
    model.add(Dropout(0.5))
    model.add(Dense(output_dim=1, input_dim=48, W_regularizer=l2(0.0001)))
...
def main():
    ...
    df_val = pd.read_hdf('db.h5', key='test', mode='r')

    # set data
    trX, trY = np.array(df.drop('y', axis=1)), np.array(df['y'])

    # set validation data
    valX, valY = np.array(df_val.drop('y', axis=1)), np.array(df_val['y'])
    ...
    # training
    history = model.fit(trX, trY, nb_epoch=100, verbose=2, validation_data=(valX, valY))
